...
- Bedrock, Google Gemini Developer API, Vertex, and Anthropic text providers.
...
| `packages/google` | partial | First-class Gemini Developer API language provider exists for API keys created in Google AI Studio, including explicit/environment auth, default/custom model paths, generate/stream, messages, tools, thought signatures, provider-owned tool-call IDs, Google options/metadata, service tiers, prompt safety blocks, usage, and supported Google/external file URLs. Batch, embeddings, files, image/video/speech/transcription/realtime, and Interactions APIs are not yet ported. |
