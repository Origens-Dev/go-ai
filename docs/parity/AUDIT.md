...
| `packages/community/openrouter` | partial | Community OpenRouter connector exists for chat generation/streaming, embeddings, provider option passthrough, streaming tool deltas, tool calls, reasoning details, and usage metadata. Fixture coverage for routing/debug/cost shapes and Anthropic-routed cache behavior remains. |
